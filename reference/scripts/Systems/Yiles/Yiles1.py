...
def Initialize():
	# Create the set ("Yiles1")
	pSet = App.SetClass_Create()
	App.g_kSetManager.AddSet(pSet, "Yiles1")

	# Save the name of the region file that's creating the set.
	pSet.SetRegionModule("Systems.Yiles.Yiles1")
	...
	# "Yiles1_S.py" file with an Initialize function that creates them.
	try:
		import Yiles1_S
		Yiles1_S.Initialize(pSet)
	except ImportError:
...
def GetSet():
	return App.g_kSetManager.GetSet("Yiles1")

def Terminate():
	App.g_kSetManager.DeleteSet("Yiles1")
...
def LoadPlacements(sSetName):
	...
	kThis.AlignToVectors(kForward, kUp)
	kThis.ConfigDirectionalLight(1, .6, .35, .75)
	kThis.Update(0)
	kThis = None
	...
	kThis.SetTranslateXYZ(0.000000, 0.000000, 0.000000)
	kForward = App.TGPoint3()
	kForward.SetXYZ(0.827170, 0.560584, -0.039190)
	kUp = App.TGPoint3()
	kUp.SetXYZ(0.028975, 0.027100, 0.999213)
	kThis.AlignToVectors(kForward, kUp)
	kThis.SetSpeed(25.000000)
	kThis.Update(0)
	kThis = None
