...
# ALL of the systems are disabled.
# The optional bDisabledIfChildrenDisabled flag, if true, will force
# this condition to check all child subsystems, and consider a subsystem
# to be disabled if all its children are disabled.
#
import App
...
class ConditionSystemDisabled:
	def __init__(self, pCodeCondition, sObject, eSystem, bDisabledIfChildrenDisabled = 0):
		# Set our parameters...
		...
		# are disabled.
		self.bDisabledIfChildrenDisabled = bDisabledIfChildrenDisabled
	...
	def SetupShipHandlers(self, pShip):
		...
		while pSubsystem:
			...
			if self.bDisabledIfChildrenDisabled:
				pInfo.FillChildInfo()
		...
		if len(self.lpWatchedSubsystems):
			...
			App.g_kEventManager.AddBroadcastPythonMethodHandler( App.ET_SUBSYSTEM_DISABLED, self.pEventHandler, "SystemDisabled", pShip )
			App.g_kEventManager.AddBroadcastPythonMethodHandler( App.ET_SUBSYSTEM_OPERATIONAL, self.pEventHandler, "SystemOperational", pShip )
	...
	def SystemDisabled(self, pEvent):
	...
	def SystemOperational(self, pEvent):
		# A system on the ship is now operational.  Is it a
		# system we care about?
		pSubsystem = App.ShipSubsystem_Cast(pEvent.GetSource())
		if pSubsystem:
			if self.dSubsystemInfoMap.has_key(pSubsystem.GetObjID()):
				...
				# disabled.
				pSubsystemInfo = self.dSubsystemInfoMap[pSubsystem.GetObjID()]
				pSubsystemInfo.bDisabled = 0
				pRoot = pSubsystemInfo.GetRoot()

				if not pRoot.IsDisabled():
					# This root is not disabled now,
