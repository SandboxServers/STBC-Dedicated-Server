# Network Debugging - Current State

## STATUS: CHECKSUM EXCHANGE STALLS AFTER REQUEST #0
The event pipeline IS connected. NewPlayerHandler fires, first checksum request is sent,
client responds. But after processing the first response, subsequent requests may not be
sent or verified correctly. Priority reliable queue gets 3 items that never clear.

## What Works (Confirmed with sendto/recvfrom IAT hooks)
- GameSpy LAN discovery: clients see server in browser
- Peek-based UDP router: demuxes GameSpy queries vs game packets on shared socket
- All singletons initialized: NetFile/ChecksumManager, MultiplayerGame, EventManager
- **NewPlayerHandler DOES fire** (INT3 breakpoints were crashing it, not proving absence)
- **TGNetwork::Send IS called** - 19+ sendtos per session
- **First checksum request IS sent** (39 bytes, opcode 0x20)
- **Client DOES respond** (28+28 bytes received)
- **At least one response IS processed** (SENDTO[5] 28 bytes sent after recv)
- Event pipeline fully connected: EventManager(0x0097F838) -> handlers at +0x2C(0x0097F864)

## Current Problem: Priority Queue Stuck at 3
After initial checksum exchange, priority reliable queue has 3 items that never ACK:
- Server retransmits 15-byte packets every ~5s (`01 D2 AA 7A 5F...`)
- Client mirrors back 15-byte responses (`02 D2 AA 7A 5F...`)
- After 8 retries (~30s), messages dropped, client times out on "Checksumming..."
...
10. ChecksumCompleteHandler (0x006a1b10) sends:
    - Opcode 0x00: [gameTime:f32][settings:2bytes][playerSlot:u8][mapNameLen:u16][mapName][passFail:u8]
...
|--------|-----------|---------|---------|
| 0x00 | Server->Client | ReceiveMessageHandler | Verification result + game settings |
| 0x01 | Server->Client | ReceiveMessageHandler | Status byte |
...
| 0x27 | ? | FUN_006a4250 | Unknown |
| 0x28 | Server->Client | (generated by FUN_006a5860) | All files transferred |

## NetFile Object (Critical Discovery)
...
## Potential Issues / Next Debugging Steps
1. **Checksum verification mismatch?** Server computes hash via FUN_0071f270/FUN_007202e0 and compares with client. If server's .pyc files differ from client's, checksum FAILS silently
2. **Client's FUN_0071f270 returns 0?** If file scan fails on client, response NOT sent (silent drop)
3. **Add logging**: Hook the NetFile ReceiveMessageHandler entry to confirm it fires for opcode 0x21
4. **Add logging**: After FUN_006a4560 processes a response, log match/mismatch result
5. **Check DAT_0097f94c**: "SkipChecksum" flag - if set, changes checksum behavior significantly
...
- sendto: HookedSendto logs first 200 outbound packets with hex dump
